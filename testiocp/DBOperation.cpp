...
_RecordsetPtr CDBOperation::ExecuteWithResSQL(const char *sql)
{
    ...
    {
        glog::trace("\nFailed to create command OR the state of connection is zero");

        if(m_pConnection->State == NULL)
        {
            //ConnToDB()
        }
...
void CDBOperation::PrintErrorInfo(_com_error &e)
{
    ...
    glog::GetInstance()->AddLine("ErrorNo: %x\nError Message:%s\nError Source:%s\nDescription:%s\n", e.Error(), (LPCTSTR)e.ErrorMessage(), (LPCTSTR) e.Source(), (LPCTSTR)e.Description());
...
BOOL CDBOperation::ReConnect(string pdir)
{
    char source[216] = {0};
    ...
    BOOL bcon = ConnToDB(source, database, uname, upass);
    return bcon;
}
